  ...
  return (
    <section id="about" className="py-20 px-4 sm:px-6 lg:px-8 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr_0.9fr] gap-6 lg:gap-12 items-center">
            {/* Left - About Me Text */}
            <motion.div
              initial={{ opacity: 0, x: -50 }}
              animate={inView ? { opacity: 1, x: 0 } : {}}
              transition={{ delay: 0.2, duration: 0.8 }}
              className="space-y-6 lg:space-y-8 bg-white p-6 lg:p-8 rounded-3xl order-2 lg:order-1"
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-3">
                {skills.map((skill, index) => (
                  <motion.div
                    ...
                    className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 rounded-lg bg-gradient-to-r from-amber-50 to-orange-50 hover:from-amber-100 hover:to-orange-100 border border-amber-200 hover:border-amber-400 shadow-sm hover:shadow-lg hover:shadow-amber-200/50 transition-all duration-300 cursor-pointer"
                  >
                    <div className="w-3 h-3 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 shadow-lg shadow-amber-300/50"></div>
                    <span className="text-gray-800 font-semibold text-sm sm:text-base">{skill.name}</span>
                  </motion.div>
                ))}
              </div>
            ...
            <motion.div
              ...
              className="bg-gradient-to-br from-amber-100 to-orange-100 rounded-3xl p-6 lg:p-8 space-y-6 lg:space-y-8 shadow-2xl border-2 border-amber-200 order-3 lg:order-3"
            >
              {stats.map((stat, index) => (
                <motion.div
                  ...
                  className="text-center py-4 lg:py-6 border-b-2 border-amber-300/30 last:border-b-0"
                >
